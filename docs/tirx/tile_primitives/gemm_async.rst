...
.. list-table::
   ...
   * - pred
     - optional runtime ``uint32`` instruction predicate; it is forwarded to
       every dense or block-scaled MMA generated by the tile call
...
Issue scope and instruction predicates
--------------------------------------

``gemm_async`` supports warp scope, which elects one issuing lane internally,
and thread scope. A thread-scope call can either be placed under an
``if T.ptx.elect_sync():`` guard, or can carry an explicit instruction
predicate while the surrounding persistent state machine remains warp-uniform:

.. code-block:: python

    mma_issue = T.ptx.elect_sync()
    Tx.gemm_async(C, A, B, SFA=SFA, SFB=SFB,
                  dispatch="tcgen05", pred=mma_issue)
    T.ptx.tcgen05.commit(mma_bar, pred=mma_issue)

The same predicate must guard the matching commit. Predicating only the commit
is incorrect: without ``pred=`` on ``gemm_async``, every active lane issues each
internal MMA. The predicate is forwarded through both the dense
``tcgen05.mma`` and block-scaled ``tcgen05.mma.block_scale`` paths.
...
.. list-table::
   ...
   * - pred
     - predicates every internally generated MMA; use the same one-lane value
       on the matching ``tcgen05.commit``
   * - M / N / K extents
