...
def test_tcgen05_16xnb_subslab_view_read(shape, rep):
    ...
    def kernel(A_ptr: T.handle, B128_ptr: T.handle, B0_ptr: T.handle, B1_ptr: T.handle) -> None:
        ...
        if wg_id == 0:
            ...
            T.tvm_storage_sync("shared")
            tmemD = T.decl_buffer(
                (128, W), dtype, scope="tmem", allocated_addr=taddr[0], layout=lay_D
            )
            tmemF0 = T.decl_buffer(
                (64, W), dtype, scope="tmem", allocated_addr=taddr[0], layout=lay_F0
            )
            tmemF1 = T.decl_buffer(
                (64, W), dtype, scope="tmem", allocated_addr=taddr[0], layout=lay_F1
            )
